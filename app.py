...
def students():
    ...
    if search:
        query += ' WHERE (name LIKE ? OR roll_number LIKE ? OR subject LIKE ?)'
...
def edit_student(id):
    conn = get_db_connection()
    student = conn.execute('SELECT * FROM students WHERE id = ? AND user_id = ?', (id, session['user_id'])).fetchone()
...
def delete_student(id):
    conn = get_db_connection()
    conn.execute('DELETE FROM students WHERE id = ? AND user_id = ?', (id, session['user_id']))
    conn.commit()
    conn.close()
...
def analytics_data():
    ...
    # Subject-wise marks
    subject_marks = df.groupby('subject')['marks'].mean().to_dict()
    
    # Marks distribution (bins: 0-40, 41-60, 61-80, 81-100)
    bins = [0, 40, 60, 80, 100]
    ...
    # Attendance vs Marks
    attendance_marks = df[['attendance', 'marks']].to_dict(orient='records')
    
    return {
        'status': 'success',
        'subjects': list(subject_marks.keys()),
        'subject_averages': list(subject_marks.values()),
...
def export_csv():
    conn = get_db_connection()
    students_data = conn.execute('SELECT name, roll_number, subject, marks, attendance FROM students').fetchall()
    conn.close()
